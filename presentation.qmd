...
## Uninformative, Simple, Merges, Splits and the Complex
### An alternative conceptulisation
...
graph {
  ...
  subgraph cluster_uninformative_simple {
    label="\"Uninformative\" and Simple (diameter: 0) "
    
    r1 [label = "HID: 1\nPID: A"]
  }
  
  subgraph cluster_merge {
    label="Merge (diameter: 1) "
...
## Features investigated {.smaller}

First reported:

+ Age (grouped: Infants; Children; Young Adults; Adults)
+ Gender (Female or Male only^[Removed records for which gender not stated/unknown impacting only the "Split" category, for which this represents 5% of records.])
+ Ethnicity
...
+ Total number of activity records (grouped into quintiles)
